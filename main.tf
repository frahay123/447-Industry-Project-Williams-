...
# Amazon SES — API sends mail using the instance role (no access keys on disk).
data "aws_iam_policy_document" "ec2_ses_send" {
  statement {
    sid    = "SesSendTransactional"
    effect = "Allow"
    actions = [
      "ses:SendEmail",
      "ses:SendRawEmail",
...
resource "aws_iam_role_policy" "ec2_ses" {
  name   = "${local.project}-ec2-ses-send"
  role   = aws_iam_role.ec2_app.id
  policy = data.aws_iam_policy_document.ec2_ses_send.json
}

# Optional: triggers verification email to ses_transactional_from_email (click link in inbox).
resource "aws_sesv2_email_identity" "transactional_from" {
  count          = var.ses_enable_email_identity ? 1 : 0
  email_identity = var.ses_transactional_from_email
}
